...
def log_validation(
    vae, text_encoders, tokenizers, unet, args, accelerator, weight_dtype, step, is_final_validation=False
):
    ...
    pipeline = StableDiffusionXLControlLoraV3Pipeline.from_pretrained(
        args.pretrained_model_name_or_path,
        vae=vae,
...
def save_model_card(repo_id: str, image_logs=None, base_model=str, repo_folder=None):
    ...
    model_description = f"""
# sdxl-control-lora-v3-{repo_id.split("sdxl-control-lora-v3-")[-1] if "sdxl-control-lora-v3-" in repo_id else repo_id}
...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Pretrained tokenizer2 name or path if not the same as model_name",
    )
    parser.add_argument(
        "--output_dir",
        type=str,
...
class TrainDataset(torch.utils.data.Dataset):
    ...
    def tokenize_captions(self, examples, is_train=True, tokenizer_idx=0):
        captions = []
        ...
        inputs = self.tokenizers[tokenizer_idx](
            captions, max_length=self.tokenizers[tokenizer_idx].model_max_length, padding="max_length", truncation=True, return_tensors="pt"
        )
    ...
    def preprocess_train(self, examples):
        ...
        examples["input_ids"] = self.tokenize_captions(examples, tokenizer_idx=0)
        examples["input_ids2"] = self.tokenize_captions(examples, tokenizer_idx=1)
        ...
        add_time_ids = [torch.tensor(add_time_ids) for _ in examples[self.image_column]]

        examples["time_ids"] = add_time_ids
...
def main(args):
    ...
    if args.tokenizer_name:
        ...
        tokenizer2 = AutoTokenizer.from_pretrained(args.tokenizer2_name, revision=args.revision, use_fast=False)
    ...
    tokenizers = [tokenizer, tokenizer2]

    # import correct text encoder class
    text_encoder_cls = import_model_class_from_model_name_or_path(args.pretrained_model_name_or_path, args.revision)
    text_encoder2_cls = import_model_class_from_model_name_or_path(args.pretrained_model_name_or_path, args.revision, subfolder="text_encoder_2")
    ...
    text_encoder2 = text_encoder2_cls.from_pretrained(
        args.pretrained_model_name_or_path, subfolder="text_encoder_2", revision=args.revision, variant=args.variant
    )
    text_encoders = [text_encoder, text_encoder2]
    ...
    def save_model_hook(models, weights, output_dir):
        if accelerator.is_main_process:
            ...
            StableDiffusionXLControlLoraV3Pipeline.save_lora_weights(
                output_dir,
